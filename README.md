...
Run the `clone.bat` script located in the root directory to clone all required repositories to your local machine. Better to run in a empty folder.
...
### 3. Module & Utility Reference

#### **A. Dependency Management Tools**

* **GeneratePomItem**: A utility for quickly generating POM XML snippets when you need to **exclude** specific JARs or manage a **dependency family** via `dependencyManagement`.
* **Workflow**: Use this in conjunction with **TreeFileScanner**.
* **Usage**: Copy the keyword search results (GroupIDs/ArtifactIDs) into `src/main/resources/Exclusion` and run the `generateExclude` or `generatedependencyManagement` methods.
* **Note**: The output is automatically de-duplicated, so you don't need to worry about redundant entries.


* **TreeFileScanner**: Scans folders for specific keywords (GroupID or ArtifactID) and generates lists to be used by the Pom generator.

#### **B. Project & Git Automation**

* **GitUtil**: A powerful class for bulk Git maintenance. It allows you to:
* Batch switch branches.
* Clean up local uncommitted changes.
* Commit and push modifications across multiple repositories simultaneously.


* **MvnUtil**: Enables you to execute `mvn clean install` across a predefined set of selected repositories in one go. Build log located in auto-cve\.log, if you find any module build failure, please check the log to see detail reason.

#### **C. Internal & System Classes**

> **Note:** These classes are generally not intended to be run as standalone tools.

* **DependencyTree**: Generates `dependency:tree` log files. (Internal use recommended).
* **LogFileSearcher**: Searches through the logs generated by the `DependencyTree` module.
* **ListTUJ**: Used for rebuilding the mapping between TUJ and API Tests. (Deprecated/Not recommended for general use).
* **Entrance**: Serves as the main entry point (Manifest Main-Class) when the `auto-cve` project is built into an executable JAR file.
...
运行项目根目录下的 `clone.bat` 脚本，克隆所有相关的 Repository 到本地。最好在一个空目录中运行。
...
* **GeneratePomItem**：用于快速生成 POM 代码块。当你需要批量 **Exclude** 特定 JAR 包，或使用 **dependencyManagement** 统一管理某个 Family 的依赖版本时非常有用。
* **配合使用**：通常配合 `TreeFileScanner` 使用。
* **操作流程**：将 `TreeFileScanner` 搜索到的关键字（GroupID/ArtifactID）拷贝到 `src/main/resources/Exclusion` 文件中，然后运行 `generateExclude` 或 `generatedependencyManagement` 方法。
* **特点**：输出结果会自动去重，无需手动清理。


* **TreeFileScanner**：用于在指定文件夹中搜索包含特定关键字（GroupID/ArtifactID）的行，为依赖清理提供基础数据。

#### **B. 项目与 Git 自动化**

* **GitUtil**：实用的 Git 批量操作类。支持：
* 批量切换分支（Switch Branch）。
* 一键清理本地修改（Clean/Reset）。
* 批量提交（Commit）并推送（Push）到远端。


* **MvnUtil**：支持对选定的一批 Repository 批量执行 `mvn clean install`，无需手动逐个编译。build日志在auto-cve\.log文件夹中，如果你看到输出结果提示有module build 失败，请查看log以了解具体原因。

#### **C. 系统及内部组件**

> **注意：** 以下类通常作为底层支撑，建议不要单独运行。

* **DependencyTree**：用于生成 `dependency:tree` 文本日志文件。
* **LogFileSearcher**：用于在 `DependencyTree` 生成的日志文件中进行检索。
* **ListTUJ**：用于重建 TUJ 与 API Test 的映射关系（目前不建议使用）。
* **Entrance**：项目打包为可执行 JAR 时的程序入口类。
